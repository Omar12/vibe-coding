...
const ShareWidget = ({ url = window.location.href, title = document.title, image = null }) => {
  ...
  const handleShare = () => {
    if (isMobile && navigator.share) {
      ...
      // Add image to share data if provided and Web Share API Level 2 is supported
      if (image && navigator.canShare && navigator.canShare({ files: [new File([""], "test.png", { type: "image/png" })] })) {
        ...
        fetch(image)
          ...
          .catch(error => console.log('Error sharing with image:', error));
      } else {
        // Fall back to basic sharing if image sharing not supported
        navigator.share(shareData).catch(error => console.log('Error sharing:', error));
      }
    } else {
      // Open overlay on desktop
      setIsOpen(true);
    }
  };
  ...
  const socialLinks = [
    {
      name: 'Twitter',
      url: `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(message ? message : title)}`,
      icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/></svg>
    },
    {
      name: 'Facebook',
      url: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}&quote=${encodeURIComponent(message)}`,
    ...
    {
      name: 'Email',
      url: `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(message ? message + '\n\n' + url : url)}`,
  ...
  return (
    <div className="share-widget">
      ...
      <button 
        onClick={handleShare}
        className="share-button flex items-center bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md"
      >
      ...
      {isOpen && !isMobile && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div 
            ref={overlayRef}
            className="share-overlay bg-white p-6 rounded-lg shadow-xl max-w-md w-full"
          >
